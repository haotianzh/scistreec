...
class ScisTreeC():
    def __init__(self, 
        ...
        if verbose:
            print('with mutation')
            print(transition.formatting(self.tran_prob_mutation))
            print('mutation free')
            print(transition.formatting(self.tran_prob_mutation_free))
    ...
    def pairwise_distance_matrix(self, probs):
        ...
        expected_distances = cp.einsum('ipq,jpq->ij', probs, 1-probs)
    ...
    def maximal_path_decoding(self, probs, tree, site_index):
        ...
        max_tree = trees[np.argmax([t.root.state[site_index, self.index_gt(2, 0)] for t in trees])]
...
if __name__ == "__main__":
    ...
    # spr move
    # tree = popgen.utils.spr_move(tree, 2)
    probs = s.init_prob_leaves(reads)
    np.save('probs2.npy', probs)
    # s.maximal_path_decoding(reads, nwk, site_index=626)
    # print(nwk)
    likelihood = s.marginal_evaluate_dp(probs, tree)
    print(likelihood)

    # likelihood = s.marginal_evaluate(probs, tree)
    # print(likelihood)